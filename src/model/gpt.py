...
class GPT(nn.Module):
    ...
    def forward(
        ...
        ckpt_active = self.gradient_checkpointing and self.training and not use_cache
            ...
            if ckpt_active:
                ...
                def _layer_fwd(x_in, mask_in, rope_cos_in, rope_sin_in, _layer=layer):
                    out, _ = _layer(
                        ...
                        kv_cache=None,
                        use_cache=False,
                        rope_cos=rope_cos_in,
                        rope_sin=rope_sin_in,
                    )
                    return out
...
if __name__ == "__main__":
    ...
    generated = generate(
        model, prompt, max_new_tokens=20, temperature=0.8, top_k=50, eos_token_id=-1
    )
    print("\n✨ 文本生成:")
