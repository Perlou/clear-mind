...
class BaseTrainer(ABC):
    ...
    def __init__(
        ...
        if use_compile and self.device.type == "cuda":
            ...
            print(
                f"⚠️  torch.compile 已请求但 device={self.device.type}，仅 CUDA 支持，已跳过"
            )
        ...
        use_fused = config.get("use_fused_adamw", True) and self.device.type == "cuda"
        adamw_kwargs = {
            "lr": self.lr,
